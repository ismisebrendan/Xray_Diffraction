...
data_filtered = ascii.read("si100_data_filtered.txt")
data_good = ascii.read("si100_data_good.txt")
...
plt.ylabel('R (s$^{-1}$)')
plt.title('Plot of the high-resolution scans both filtered and unfiltered of Si(100)')
plt.legend()
plt.show()
...
plt.xlabel(r'$\beta$ ($^{\circ}$)')
plt.ylabel('R (s$^{-1}$)')
plt.title('Plot of the high-resolution diffracton patterns both filtered and unfiltered of Si(100) with peaks fitted')


peak1 = np.arange(12.4,14.4,0.1)
p1 = np.rint((peak1-3)*10).astype(int)
fit_peak1 = opt.curve_fit(gaussian, peak1, rate_good[p1], p0=[100, 12.5, 3, 60])[0]
a, b, c, d = fit_peak1
...
peak2 = np.arange(14.7,16.4,0.1)
p2 = np.rint((peak2-3)*10).astype(int)
fit_peak2 = opt.curve_fit(gaussian, peak2, rate_good[p2], p0=[300, 15, 1, 60])[0]
a, b, c, d = fit_peak2
...
peak5 = np.arange(23.5,26,0.1)
p5 = np.rint((peak5-3)*10).astype(int)
fit_peak5 = opt.curve_fit(gaussian, peak5, rate_good[p5], p0=[100, 24, 1, 20])[0]
a, b, c, d = fit_peak5
...
peak6 = np.arange(26,30,0.1)
p6 = np.rint((peak6-3)*10).astype(int)
fit_peak6 = opt.curve_fit(gaussian, peak6, rate_good[p6], p0=[100, 27.5, 1, 20])[0]
a, b, c, d = fit_peak6
...
peak7 = np.arange(30,34,0.1)
p7 = np.rint((peak7-3)*10).astype(int)
fit_peak7 = opt.curve_fit(gaussian, peak7, rate_good[p7], p0=[100, 31, 1, 20])[0]
a, b, c, d = fit_peak7
